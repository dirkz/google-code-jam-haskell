...
module InfiniteHouseOfPancakes where
...
solve c@(TestCase n plates) = optimum plates

--optimum :: [Int] -> Int
optimum plates = res
  where
    maxPancakes = maximum plates
    xs = [1..maxPancakes]
    moves = map (countMoves plates) xs
    minn = minimum $ map snd moves
    res = min maxPancakes minn

countMoves :: [Int] -> Int -> (Int, Int)
countMoves xs c = (c, c + (sum $ map calc xs))
  where
    calc y = floor $ fromIntegral (y - 1) / fromIntegral c
